...
class track_inst{
	...
	public void write(){
		System.out.println(op_code+s_fs);
	}
}
...
class track_reg{
	...
	int regval;
        track_reg(){
            //System.out.println("im register memory");
        }
	track_reg(String str1,int ins){
		regname = str1;
		regval = ins;
	}
}
...
class track_memdata{
	...
	track_memdata(){
            //System.out.println("im in data memory");
	}
	track_memdata(int a,int v){
		dataadd = a;
		dataval = v;
	}
}
...
class Simulator {
	...
	ArrayList<track_inst> INM = new ArrayList<track_inst>();
	
	BufferedReader buf_read3 = null;
	ArrayList<track_memdata> DAM = new ArrayList<track_memdata>();
	
	track_memdata data_mem = null;
	
	File simulation_file;
	
	File register_file;
	ArrayList<track_inst> SIB = new ArrayList<track_inst>();
	File data_file;
	...
	ArrayList<track_reg> RGF = new ArrayList<track_reg>();
	BufferedReader buf_read2 = null;
	
	ArrayList<track_inst> INB = new ArrayList<track_inst>();
	
	File instruction_file;
	ArrayList<track_inst> AIB = new ArrayList<track_inst>();
	
	ArrayList<track_inst> PRB = new ArrayList<track_inst>();
	ArrayList<track_reg> ADB = new ArrayList<track_reg>();
	BufferedWriter writein_file; 
	
	ArrayList<track_reg> REB = new ArrayList<track_reg>();
	...
	public void Input_Registers(){
		...
		public void Input_Datamemory(){
		  ...
		  while((cur_line = buf_read3.readLine())!= null)
		  {
			  ...
			  data_mem = new track_memdata(add,val);
			  DAM.add(data_mem);
			}
		}catch(Exception e){System.out.print(e);}
	...
	public void main_task() throws IOException {
		...
		try{
		  ...
		  if (!simulation_file.exists()) {
				...
				w_file = new FileWriter(simulation_file.getAbsoluteFile());
				...
				line_print();//prints current status of all the lists
	...
	public void decode_read(track_inst in){
		if (in.op_code.equalsIgnoreCase("ADD") || in.op_code.equalsIgnoreCase("MUL") || in.op_code.equalsIgnoreCase("SUB")){
			for (int i = 0; i<RGF.size() ; i++){
				if (RGF.get(i).regname.equals(in.f_fs))
					in.f_fs = String.valueOf(RGF.get(i).regval);
				if (RGF.get(i).regname.equals(in.s_fs))
					in.s_fs = String.valueOf(RGF.get(i).regval);
			}
		}
		else{
			for (int i = 0; i<RGF.size() ; i++){
	...
	public void Unit_add_sub(track_inst in){
		track_reg r = new track_reg() ;
		if (in.op_code.equalsIgnoreCase("ADD")){
			r.regval = Integer.parseInt(in.f_fs) + Integer.parseInt(in.s_fs);
			r.regname = in.destination_value;
			REB.add(r);
			//AIB.op_code = "null";
		}
	...
	public void Unit_mul(track_inst prb){
		track_reg rebtemp = new track_reg();
		rebtemp.regval =  Integer.parseInt(prb.f_fs) * Integer.parseInt(prb.s_fs);
		rebtemp.regname = prb.destination_value;
		REB.add(rebtemp);
	...
	public void add_pnh(track_inst sib){
		track_reg tempadb = new track_reg();
		tempadb.regval =  Integer.parseInt(sib.f_fs) + Integer.parseInt(sib.s_fs);
		tempadb.regname = sib.destination_value;
		ADB.add(tempadb);
	...
	public int position_DAM(track_memdata dat){
	...
	public void str_data(track_reg adb){
		boolean found = false;
		int rval = 0;
		track_memdata d = new track_memdata();
		for (int i = 0; i<RGF.size() ; i++){
	...
	public int find_position(track_reg temp){
	...
	public void Unit_w(track_reg reb)
	{
	...
	public void line_print() throws IOException{
		...
		writein_file.write("ADB:");
                if(0<ADB.size()){
                if(ADB.get(0).regname.equals("null")){
		    	Sret = "";
		    }
		    else{
			Sret = '<'+ADB.get(0).regname+','+ADB.get(0).regval+'>';
		...
		writein_file.write("REB:");
                if(0<REB.size()){
                if(REB.get(0).regname.equals("null")){
		    	Sret = "";
		    }
		    else{
			Sret = '<'+REB.get(0).regname+','+REB.get(0).regval+'>';
		...
		writein_file.write("RGF:");
                ...
                int k=0;
               if(k<RGF.size()){
                  if(RGF.get(k).regname.equals("null")){
		    	Sret = "";
		    }
		    else{
			Sret = '<'+RGF.get(k).regname+','+RGF.get(k).regval+'>';
		...
		writein_file.write("DAM:");
                int l=0;
                if(l<DAM.size()){
                    if(String.valueOf(DAM.get(l).dataadd).equals("null")){
		    	Sret = "";
		    }
		    else{
			Sret = "<"+DAM.get(l).dataadd+","+DAM.get(l).dataval+">";
